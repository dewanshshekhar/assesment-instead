...
A self-contained editor: the blank form, every box drawn on top of it, drag to
move, handles to resize, arrow keys to nudge a point at a time, shift-drag on
bare paper to draw a box the PDF never declared, and the edited template back
out as JSON. Rows generated by a repeat appear dashed and are not draggable,
so a region that is already covered looks covered.

Importing an AcroForm gets most boxes for free, but every form has some it does
not declare — Form 1040's "more than four dependents" check is one — and
placing those by editing coordinates in JSON and re-rendering to see what moved
is the slowest part of the work.
